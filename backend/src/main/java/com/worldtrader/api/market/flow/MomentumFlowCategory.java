...
public class MomentumFlowCategory implements OrderFlowCategory {
    ...
    public double intensity(OrderFlowContext context) {
        ...
        return 0.8 + 4.0 * Math.abs(m.nofi()) + 2.0 * context.regime().getBurstiness() + 0.8 * context.regime().getNewsIntensity();
    }
    ...
    public List<OrderIntent> generateOrders(OrderFlowContext context, double dtSeconds, int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            out.add(new OrderIntent("FLOW_MOMO", context.ticker(), side, OrderType.MARKET, qty, null, TimeInForce.IOC));
        }
        return out;
    }
}
