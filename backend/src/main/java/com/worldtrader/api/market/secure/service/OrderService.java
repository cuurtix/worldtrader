...
public class OrderService {
    ...
    public OrderService(OrderRepository orderRepository, BalanceRepository balanceRepository, InstrumentRepository instrumentRepository,
                        BalanceAuditRepository balanceAuditRepository, StringRedisTemplate redisTemplate, MeterRegistry meterRegistry) {
    ...
    public void cacheInstrumentConfig(String symbol, String payload) {
        redisTemplate.opsForValue().set("instrument:config:" + symbol, payload, Duration.ofHours(1));
    }

    public void cacheUserProfile(String userId, String payload) {
        redisTemplate.opsForValue().set("user:profile:" + userId, payload, Duration.ofMinutes(5));
    }
